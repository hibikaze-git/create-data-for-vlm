...
for test
python calm3_batch_cc-by-sa.py team-hatakeyama-phase2/commoncatalog-cc-by-sa-ja --num_samples 10 --batch_size 2 --tensor_parallel_size 1
"""
...
class Translater:
    ...
    def translate_dataset(self, dataset, batch_size, num_samples):
        ...
        for i in tqdm(range(0, len(dataset), batch_size), desc="バッチ処理中"):
            ...
            en_texts = batch["briefly"]

            briefly_translated_texts = self.translate_batch(
                [TRANSLATE_PROMPT.format(text=text) for text in en_texts]
            )

            en_texts = batch["detail"]

            detail_translated_texts = self.translate_batch(
                [TRANSLATE_PROMPT.format(text=text) for text in en_texts]
            )

            en_texts = batch["predict"]

            predict_translated_texts = self.translate_batch(
                [TRANSLATE_PROMPT.format(text=text) for text in en_texts]
            )

            for (
                data,
                briefly_translated_text,
                detail_translated_text,
                predict_translated_text,
            ) in zip(
                converted_data,
                briefly_translated_texts,
                detail_translated_texts,
                predict_translated_texts,
            ):
                data["briefly_ja_calm3"] = briefly_translated_text
                data["detail_ja_calm3"] = detail_translated_text
                data["predict_ja_calm3"] = predict_translated_text
